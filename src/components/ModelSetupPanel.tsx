import { CircleCheck, Download, ExternalLink, LoaderCircle, RotateCw, Trash2, TriangleAlert } from 'lucide-react'
...
export function ModelSetupPanel({ status, progress, isBusy, onInstall, onRemove }: ModelSetupPanelProps) {
  const percent = progress ? Math.min(100, Math.round(progress.downloadedBytes / progress.totalBytes * 100)) : 0
  const canInstall = status.state !== 'ready' && status.state !== 'unavailable'
  ...
  return (
    <section className="model-setup-panel" aria-label="本地 WD14 模型">
      <div className="model-setup-heading">
        <span>{status.state === 'ready' ? <CircleCheck size={16} /> : <TriangleAlert size={16} />}</span>
      ...
      {status.state !== 'unavailable' ? <>
        ...
        <div className="model-actions">
          ...
          {status.installedVersion ? <button className="model-remove" type="button" disabled={isBusy} onClick={onRemove}><Trash2 size={14} />移除模型</button> : null}
        </div>
